...
function Tween(obj, t, data)
    if not obj or not services.TweenService then return false end
...
-- 主要的UI创建函数
function X.new(libraryName, theme)
    -- 清理旧的UI
    ...
    local success = pcall(function()
        ...
        elseif getexecutorname and protect_gui then
            protect_gui(dogent)
        elseif gethui then
    ...
    ScriptTitle.Text = libraryName or "X UI Library"
    ...
    function window:Tab(name, icon)
        ...
        function tab:section(name, TabVal)
            ...
            function section:Toggle(text, flag, enabled, callback)
                ...
                ToggleDisable.Position = UDim2.new(0.902, 0, 0.209, 0)
                ...
                local funcs = {
                    SetState = function(self, state)
                        if state == nil then state = not X.flags[flag] end
                        if X.flags[flag] == state then return end
                        Tween(ToggleSwitch, {0.2, 'Linear', 'InOut'}, {
            ...
            function section:Slider(text, flag, default, min, max, precise, callback)
                ...
                MinSlider.Position = UDim2.new(0.297, 40, 0.237, 0)
                ...
                AddSlider.Position = UDim2.new(0.811, 0, 0.5, 0)
                ...
                MinSlider.MouseButton1Click:Connect(function()
                    local currentValue = X.flags[flag]
                    ...
                    for _, option in next, DropdownModule:GetChildren() do
                        if option:IsA("TextButton") and option.Name:match("Option_") then
                            if text == "" then
                                ...
                                option.Visible = option.Text:lower():match(text:lower())
                            end
                        end
                    end
                end
