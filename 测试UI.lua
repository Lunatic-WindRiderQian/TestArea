...
function FengUI.new(FengUI, name, theme)
    ...
    function window.card(window, name, description, icon)
        ...
        function createTab(cardObj, tabName, tabIcon, windowCount)
            ...
            -- 现代化的折叠面板 Section (移植自 Eternal 库，适配 FengUI 配色)
            function tabObj.section(tabObj, name, windowPosition, TabVal)
                ...
                HeaderText.TextColor3 = config.AccentColor
                ...
                ArrowIcon.Image = "rbxassetid://134243273101015"  -- chevron-down 图标
                ...
                -- 原 Objs 容器（保持原有元素添加逻辑）
                local Objs = Instance.new("Frame")
                Objs.Name = "Objs"
                Objs.Parent = ContentFrame
                ...
                local function updateSectionHeight()
                    ...
                    local contentHeight = ObjsL.AbsoluteContentSize.Y + ContentPadding.PaddingTop.Offset + ContentPadding.PaddingBottom.Offset
                ...
                local function setOpen(state)
                    ...
                    if open then
                        ...
                        local contentHeight = ObjsL.AbsoluteContentSize.Y + ContentPadding.PaddingTop.Offset + ContentPadding.PaddingBottom.Offset
                        local targetHeight = 36 + contentHeight
                        services.TweenService:Create(SectionFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            Size = UDim2.new(1, 0, 0, targetHeight)
                        }):Play()
                        services.TweenService:Create(ContentFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            Size = UDim2.new(1, 0, 0, contentHeight)
                        }):Play()
                        services.TweenService:Create(ArrowIcon, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
                            Rotation = 0
                        }):Play()
                    else
                        -- 折叠：收起高度
                        ...
                        services.TweenService:Create(ContentFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ...
                HeaderBtn.MouseButton1Click:Connect(function()
                    DigitalParticleExplosion(HeaderBtn)
                    setOpen(not open)
                end)
                
                -- 鼠标悬停效果
                HeaderBtn.MouseEnter:Connect(function()
                    services.TweenService:Create(HeaderText, TweenInfo.new(0.2), {
                        TextColor3 = config.TextColor
                    }):Play()
                    services.TweenService:Create(ArrowIcon, TweenInfo.new(0.2), {
