...
function FengUI.new(FengUI, name, theme)
    ...
    setupSmoothScrolling(RightContainer, RightLayout)
        ...
        function tab.section(tab, name, windowPosition, TabVal)
            ...
            local isTabValBoolean = false
            
            -- 检测TabVal是否是布尔值（可能是第一个参数）
            if type(windowPosition) == "boolean" then
                TabVal = windowPosition
                windowPosition = "Left"
                isTabValBoolean = true
            elseif windowPosition == true or windowPosition == false then
                TabVal = windowPosition
                windowPosition = "Left"
                isTabValBoolean = true
            end
            ...
            -- 修复：正确处理TabVal参数
            local open = true  -- 默认打开
            if TabVal ~= nil then
                if type(TabVal) == "boolean" then
                    open = TabVal
                elseif type(TabVal) == "string" then
                    if TabVal:lower() == "false" or TabVal == "0" or TabVal:lower() == "off" then
                        open = false
                    elseif TabVal:lower() == "true" or TabVal == "1" or TabVal:lower() == "on" then
                        open = true
                    end
                elseif type(TabVal) == "number" then
                    open = TabVal ~= 0
                end
            end
            ...
            local function applyInitialState()
                if open then
                    ...
                    SectionOpened.ImageTransparency = 0
                    SectionOpen.ImageTransparency = 1
                else
                    Section.Size = UDim2.new(1, 0, 0, 36)
                    SectionOpened.ImageTransparency = 1
                    SectionOpen.ImageTransparency = 0
                end
            end
            
            -- 初始应用
            task.spawn(function()
                task.wait(0.05)  -- 等待一小段时间让UI布局更新
                applyInitialState()
            end)
            ...
            ObjsL:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                if open then
                    -- 只有在打开状态下才更新高度
                    local targetHeight = 36 + ObjsL.AbsoluteContentSize.Y + 8
                    services.TweenService:Create(Section, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Size = UDim2.new(1, 0, 0, targetHeight)
                    }):Play()
                end
            end)
