...
-- ==================== 新测试UI.lua（适配层） ====================
-- 保留原测试UI.lua的API，但使用maclib.lua作为底层UI实现

local Library = {}  -- 保持原导出表名
...
local ConfigObjects = {}  -- 保留原配置对象表

-- 保留原主题/彩虹相关函数（但maclib不支持，留空或简单实现）
function Library:SetTheme(themeName) end
function Library:ToggleRainbow(bool) end
function Library:SetRainbowType(val) end
...
function Library:CreateWindow(Config)
    ...
    local win = MacLib:Window({
        Title = Config.Title or "M0dzn UI",
        Subtitle = "",  -- maclib需要subtitle，可留空
        Size = UDim2.fromOffset(450, 280),  -- 与原测试UI一致
        DragStyle = 1,
    ...
    -- 为窗口创建一个TabGroup（maclib要求先有TabGroup才能添加Tab）
    local tabGroup = win:TabGroup()
    local macTabs = {}  -- 存储maclib的tab对象，key为标签页名称
    local currentSideSections = {}  -- 存储每个tab当前使用的Section（我们固定使用左侧）
    ...
    function Window:Tab(name, icon)
        -- 在maclib中创建对应标签页
        local macTab = tabGroup:Tab({ Name = name, Image = icon and tostring(icon) or "" })
        macTabs[name] = macTab

        -- 为这个tab创建一个Section（左侧），所有元素将添加到此Section
        local section = macTab:Section({ Side = "Left" })
        currentSideSections[name] = section

        -- 返回Elements表，包含原测试UI的所有元素创建方法
        local Elements = {}
        ...
        -- Toggle
        function Elements:Toggle(toggleText, default, callback)
            ...
            section:Toggle({
                ...
                Callback = function(state)
                    ...
                    ConfigObjects[toggleText] = { Type = "Toggle", Value = state }
                    if callback then callback(state) end
                    Window:Notification(toggleText..": "..tostring(state))
                end
            })
        end

        -- Slider
        function Elements:Slider(sliderText, min, max, default, callback)
            ...
            section:Slider({
                ...
                Callback = function(v)
                    ...
                    ConfigObjects[sliderText] = { Type = "Slider", Value = v }
                    if callback then callback(v) end
                end
            })
        end

        -- Textbox (对应maclib的Input)
        function Elements:Textbox(boxText, placeholder, callback)
            section:Input({
                Name = boxText,
                Placeholder = placeholder or "",
                Default = "",
                Callback = function(text)
                    ConfigObjects[boxText] = { Type = "Textbox", Value = text }
        ...
        -- Dropdown
        function Elements:Dropdown(dropText, options, callback)
            local dropdownObj = section:Dropdown({
                Name = dropText,
                Options = options,
                Multi = false,
                Required = false,
                ...
                Callback = function(selected)
                    ConfigObjects[dropText] = { Type = "Dropdown", Value = selected }
                    if callback then callback(selected) end
                end
            })
            -- 返回一个包含Refresh方法的表，以兼容原测试UI的用法（可选）
            return { Refresh = function(newOpts) dropdownObj:InsertOptions(newOpts) end }
        end

        -- Keybind
        function Elements:Keybind(keyText, default, callback)
            ...
            section:Keybind({
                ...
                onBinded = function(bind)
                    key = bind
                    ConfigObjects[keyText] = { Type = "Keybind", Value = bind.Name }
                    Window:Notification("Keybind: "..bind.Name)
                end
            })
        end

        -- Value (对应maclib的Input)
        function Elements:Value(valText, default, callback)
            section:Input({
                Name = valText,
                Default = tostring(default or ""),
                Callback = function(text)
                    PlaySound(Sounds.Click)
                    ConfigObjects[valText] = { Type = "Value", Value = text }
                    if callback then callback(text) end
                    Window:Notification(valText..": "..text)
                end
            })
        end

        -- Section (可折叠分组)  - 由于maclib的Section不支持折叠且已有边框，我们直接返回Elements本身，或创建新Section
        -- 为尽量保持原API，我们让Section方法创建一个新的maclib Section并返回其元素创建表
        function Elements:Section(text, icons, defaultOpen)
            -- 忽略icons和defaultOpen，创建新Section
            local newSection = macTab:Section({ Side = "Left" })
            -- 添加一个Header来模拟原Section标题
            newSection:Header({ Text = text })
            -- 返回这个新Section的元素创建表（与Elements结构相同）
            return {
                Button = function(_, btnText, cb) newSection:Button({ Name = btnText, Callback = cb }) end,
                Toggle = function(_, toggleText, def, cb) newSection:Toggle({ Name = toggleText, Default = def, Callback = cb }) end,
                Slider = function(_, sliderText, min, max, def, cb) newSection:Slider({ Name = sliderText, Minimum = min, Maximum = max, Default = def, Callback = cb }) end,
                Textbox = function(_, boxText, placeholder, cb) newSection:Input({ Name = boxText, Placeholder = placeholder, Callback = cb }) end,
                Dropdown = function(_, dropText, opts, cb) newSection:Dropdown({ Name = dropText, Options = opts, Callback = cb }) end,
                Keybind = function(_, keyText, def, cb) newSection:Keybind({ Name = keyText, Default = def, Callback = cb }) end,
                Value = function(_, valText, def, cb) newSection:Input({ Name = valText, Default = tostring(def), Callback = cb }) end,
                Section = Elements.Section,  -- 支持嵌套（递归）
            }
        end
