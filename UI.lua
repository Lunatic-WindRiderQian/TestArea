...
RightEffects.BackgroundColor3 = Color3.fromRGB(5, 5, 15)
...
borderLines.left.Name = "BorderLeft"
borderLines.left.Parent = RightEffects
borderLines.left.BackgroundColor3 = Color3.fromRGB(0, 200, 255)
borderLines.left.Size = UDim2.new(0, 1, 1, 0)
borderLines.left.ZIndex = 10

borderLines.right.Name = "BorderRight"
borderLines.right.Parent = RightEffects
borderLines.right.BackgroundColor3 = borderLines.left.BackgroundColor3
borderLines.right.Size = UDim2.new(0, 1, 1, 0)
borderLines.right.Position = UDim2.new(1, -1, 0, 0)
borderLines.right.ZIndex = 10

-- █ 保留元素2：数据矩阵（优化版）
local matrixColumns = {}
for i = 1, 12 do  -- 列数从15减到12
    local col = Instance.new("Frame")
    col.Name = "MatrixCol_"..i
    col.Parent = RightEffects
    col.Size = UDim2.new(0, 8, 1, 0)
    col.Position = UDim2.new((i-1)/11, 0, 0, 0)
    
    local chars = {}
    for j = 1, 15 do  -- 每列字符从20减到15
        ...
        char.TextSize = 12
        char.Size = UDim2.new(1, 0, 0, 15)
        char.Position = UDim2.new(0, 0, (j-1)/15, 0)
...
-- █ 保留元素4：核心脉冲节点
local pulseNodes = {}
for i = 1, 6 do  -- 节点从12减到6
    local node = Instance.new("ImageLabel")
    node.Name = "PulseNode_"..i
    node.Parent = RightEffects
    node.Image = "rbxassetid://7733960981"
    node.ImageColor3 = Color3.fromRGB(0, 150, 255)
    node.ImageTransparency = 0.3
    node.Size = UDim2.new(0, 10, 0, 10)
    ...
    glow.Name = "Glow"
    glow.Parent = node
    glow.Image = "rbxassetid://7733765398"
    glow.ImageColor3 = node.ImageColor3
...
local function optimizedUpdate()
    ...
    if frameCounter % 4 == 0 then
        local pulse = (math.sin(os.clock()) + 1) / 2
        borderLines.left.BackgroundTransparency = 0.3 + pulse*0.5
        borderLines.right.BackgroundTransparency = borderLines.left.BackgroundTransparency
    end
    
    -- 数据矩阵（降频到20FPS）
    if frameCounter % 3 == 0 then
        for _, col in ipairs(matrixColumns) do
            col.headPos = col.headPos + col.speed * 0.1
            for i, char in ipairs(col.chars) do
                local pos = (i - col.headPos) / 12
            ...
            if col.headPos > #col.chars + 5 then
                col.headPos = -math.random(5, 10)
            end
        end
    end
    
    -- 扫描线（保持60FPS）
    scanLine.Position = scanLine.Position + UDim2.new(0, 0, 0.003, 0)
    if scanLine.Position.Y.Scale > 1.2 then
        scanLine.Position = UDim2.new(0, 0, -0.2, 0)
    end
    
    -- 脉冲节点（降频到15FPS）
    if frameCounter % 4 == 0 then
        for _, node in ipairs(pulseNodes) do
            node.timer = node.timer + 0.1
            node.obj.PulseRing.Size = UDim2.new(
                3 + math.sin(node.timer)*0.5, 
                0, 
                3 + math.sin(node.timer)*0.5, 
                0
            )
            node.obj.PulseRing.ImageTransparency = 0.7 + math.sin(node.timer)*0.2
        end
    end
end
