...
function Ripple(obj)
    ...
    task.spawn(function()
        ...
        services.TweenService:Create(Ripple, TweenInfo.new(0.3, Enum.EasingStyle.Linear), {
...
function switchTab(new)
    ...
    services.TweenService:Create(old[1], tweenInfo, { ImageTransparency = 0.2 }):Play()
    services.TweenService:Create(new[1], tweenInfo, { ImageTransparency = 0 }):Play()
    services.TweenService:Create(old[1].TabText, tweenInfo, { TextTransparency = 0.2 }):Play()
...
local lastRainUpdate = 0
RunService.Heartbeat:Connect(function(deltaTime)
    lastRainUpdate = lastRainUpdate + deltaTime
    if lastRainUpdate < 0.03 then return end
    lastRainUpdate = 0
...
for i = 1, 12 do
    ...
    column.Position = UDim2.new((i-1)/11, 0, 0, 0)
    ...
    for j = 1, 15 do
        ...
        char.Position = UDim2.new(0, 0, (j-1)/14, 0)
...
local pulsePhase = 0
RunService.Heartbeat:Connect(function(deltaTime)
    lastEffectUpdate = lastEffectUpdate + deltaTime
    if lastEffectUpdate < 0.016 then return end
    lastEffectUpdate = 0
    ...
    for _, particle in ipairs(blueParticles) do
        local newX = particle.basePos.x + math.sin(os.clock() * 0.5) * particle.floatIntensity
        local newY = particle.basePos.y + math.cos(os.clock() * 0.7) * particle.floatIntensity
    ...
    for _, column in ipairs(matrixColumns) do
        ...
        for i, char in ipairs(column.chars) do
            ...
            if pos >= 0 and pos <= 1 then
                ...
                char.TextColor3 = Color3.fromRGB(0, 255 - pos * 155, 100 + pos * 100)
            else
                char.TextTransparency = 1
            end
    ...
    function window.Tab(window, name, icon)
        ...
        function tab.section(tab, name, TabVal)
            ...
            function section.Toggle(section, text, flag, enabled, callback)
                ...
                ToggleSwitch.Position = UDim2.new(0, enabled and ToggleSwitch.Size.X.Offset/2 or 0, 0, 0)
