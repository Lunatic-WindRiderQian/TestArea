--[[
    ...
    - Slider = miUI 单行紧凑布局（名称 + 滑轨 + 数值框同一行）
    - 其他控件 = miUI 风格视觉框架
...
local function createSectionBuilder(parent, contentContainer, elementWidth, windowCount, window)
    ...
    child.Slider = function(_, config)
        ...
        -- 内部行高 16
        local rowH = 16
        local rowY = (tileH - rowH) / 2

        -- 左侧：名称（宽 90）
        local TitleLbl = miLabel(Tile, sliderText, 15, rowY, UDim2.new(0, 90, 0, rowH), 12)

        -- 右侧：数值框（宽 60 / 46，高 16）
        local numW = unlimited and 60 or 46
        local ValueFrame = Instance.new("Frame")
        ValueFrame.Size = UDim2.new(0, numW, 0, 16)
        ValueFrame.Position = UDim2.new(1, -(numW + 10), 0.5, -8)
        ...
        -- 中间：滑轨（4 高，圆点 8）
        local trackLeft = 15 + 90 + 10
        local trackRight = numW + 10 + 10
        local Track, Fill, Knob, Bar
        if not unlimited then
            Track = Instance.new("Frame")
            Track.Size = UDim2.new(1, -(trackLeft + trackRight), 0, 4)
            Track.Position = UDim2.new(0, trackLeft, 0.5, -2)
...
function Fenglib:CreateWindow(Config)
    ...
    function Window:Category(config)
        ...
        task.spawn(function()
            task.wait()
            local h = getContentHeight()
            content.Size = UDim2.new(1, 0, 0, opened and h or 0)
            updateTabCanvas()
        end)
        Window._currentCategory = {frame = categoryFrame, content = content, contentList = contentList, header = header, arrow = arrow, collapsible = collapsible, opened = opened}
