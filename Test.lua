...
function Fenglib:CreateWindow(Config)
    ...
    -- 卡片模式相关变量
    local CardsScroll = nil
    local CardsGridContainer = nil
    local CardsGridLayout = nil
    local cardList = {}          -- 存储所有卡片对象 { frame, titleLabel, subLabel, imageLabel, callback }
    local function updateCardsGridLayout()
        if not CardsGridContainer or not CardsGridLayout then return end
        local containerWidth = CardsGridContainer.AbsoluteSize.X
        if containerWidth <= 0 then return end
        local padding = 10
        local cardWidth = (containerWidth - padding * 3) / 2   -- 每行两个卡片
        cardWidth = math.max(180, cardWidth)
        CardsGridLayout.CellSize = UDim2.new(0, cardWidth, 0, 110)
    end

    local function refreshCardsTheme()
        for _, card in ipairs(cardList) do
            if card.frame and card.frame.Parent then
                card.frame.BackgroundColor3 = CurrentTheme.Top
                if card.frame:FindFirstChild("UIStroke") then
                    card.frame.UIStroke.Color = CurrentTheme.Stroke
                end
                card.titleLabel.TextColor3 = CurrentTheme.Text
                card.subLabel.TextColor3 = CurrentTheme.Text
            end
        end
    end
    table.insert(ThemeListeners, refreshCardsTheme)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if isResizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            if useCardMode then updateCardsGridLayout() end
        end
    end)
    ...
    if useCardMode then
        -- 卡片滚动区域
        CardsScroll = Instance.new("ScrollingFrame")
        ...
        CardsGridContainer = Instance.new("Frame")
        ...
        CardsGridContainer:GetPropertyChangedSignal("AbsoluteSize"):Connect(onGridSizeChanged)
        MainFrame:GetPropertyChangedSignal("Size"):Connect(onGridSizeChanged)
        task.spawn(onGridSizeChanged)
        ...
        function Window:card(mainText, subText, imageId, callback)
            ...
            cardFrame.Size = UDim2.new(1, 0, 1, 0)  -- 实际大小由GridLayout决定
            ...
            clickBtn.MouseButton1Click:Connect(function()
                ...
                if callback then callback(cardFrame) end
            end)

            -- 储存在列表中
            local cardData = { frame = cardFrame, titleLabel = mainLabel, subLabel = subLabel, imageLabel = leftImage, callback = callback }
