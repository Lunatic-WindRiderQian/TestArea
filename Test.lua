...
local function createSectionBuilder(parent, contentContainer, elementWidth, windowCount)
    local function createSection(text, icons, defaultOpen)
        ...
        child.Input = function(_, inputText, default, callback, options)
            ...
            local InputFrame = Instance.new("Frame"); InputFrame.Size = UDim2.new(1, 0, 0, 42); InputFrame.Parent = contentContainerSection; InputFrame.BackgroundTransparency = 0.05; Instance.new("UICorner", InputFrame).CornerRadius = UDim.new(0, 12); AddToRegistry(InputFrame, "BackgroundColor3", "Top")
...
function Fenglib:CreateWindow(Config)
    ...
    -- === NEW MODERN MAIN FRAME (inspired by metUI.lua) ===
    ...
    Instance.new("UICorner", MainFrame).CornerRadius = UDim.new(0, WINDOW_CORNER_RADIUS)
    ...
    -- Icon
    if IconAsset and tonumber(IconAsset) then
        IconAsset = "rbxassetid://" .. IconAsset
    elseif not IconAsset then
    ...
    ButtonGroup.Size = UDim2.new(0, 120, 1, 0)
    ButtonGroup.Position = UDim2.new(1, -130, 0, 0)
    ...
    -- === CONTENT AREA (original Fenglib layout) ===
    ...
    -- 页面容器（改为 CanvasGroup 的父容器）
    local PageContainer = Instance.new("Frame")
    PageContainer.Size = UDim2.new(1, -165, 1, 0)
    PageContainer.Position = UDim2.new(0, 160, 0, 0)
    PageContainer.BackgroundTransparency = 1
    PageContainer.ClipsDescendants = true      -- 裁剪溢出部分
    PageContainer.Parent = Content

    local firstTab = true
    local activeCanvas = nil                   -- 当前显示的 CanvasGroup

    -- 切换 Tab 的函数（动画风格来自 metUI）
    ...
    function Window:Tab(name, icon)
        ...
        if icon then
            ...
            TabIcon.Image = tonumber(icon) and "rbxassetid://" .. icon or icon
        ...
        -- ===== 页面 CanvasGroup（包裹 ScrollingFrame）=====
    ...
    cornerStroke.Color = Color3.new(1, 1, 1)
    cornerStroke.Transparency = 0
    cornerStroke.Parent = Resizer

    local cornerCorner = Instance.new("UICorner")
    cornerCorner.CornerRadius = UDim.new(0, 6)
    cornerCorner.Parent = Resizer
    ...
    -- === FLOATING OPEN BUTTON (unchanged) ===
    ...
    -- === PROJECTOR MODE SUPPORT (unchanged but adapted to new MainFrame) ===
    ...
    if isCardMode then
        ...
        local BackButton = createIconButton("rbxassetid://7733764800", function()
            showCardList()
        end)
    ...
    -- 注意：Card 模式保留原有切换方式，未做动画修改，但普通 Tab 已替换为 metUI 风格。

    -- === PUBLIC WINDOW METHODS (unchanged) ===
